...
Disc #6 has 19 positions; at time=0, it is at position 17.].split("\n")

# input = %[Disc #1 has 5 positions; at time=0, it is at position 4.
# Disc #2 has 2 positions; at time=0, it is at position 1.].split("\n")

if !PART_ONE
  input << "Disc #7 has 11 positions; at time=0, it is at position 11."
end

def parse(line)
  /Disc #(\d+) has (\d+) positions; at time=0, it is at position (\d+)/ =~ line
  # disk     positions  start
  [ $1.to_i, $2.to_i,   $3.to_i ]
end

lines = input.map {|l| parse(l)}

# BRUUUUUUUTE FOOOOOOORRRRRCE
def fall?(sculpture, time)
  orientation = 0
  sculpture.each do |level|
    orientation += (level[2] + time + level[0]) % level[1]
...
10000000.times do |n|
  if fall?(lines, n)
    puts "FALLS FREE AT #{ n } SECONDS AFTER START"
    break
  end
end
