...
def checksum(val)
  ...
  check = []
  while idx < val.size - 1
    check << (val[idx] === val[idx + 1] ? '1' : '0')
    idx += 2
  end
...
puts "START:       #{ input.join('') }"
while input.size < NEEDS
  input = evolve(input)
  count += 1
  ...
  if input.size > NEEDS
    puts "GOT..."
    input = input[0, NEEDS]
    puts "FINAL:       #{ input.size } bits"
    puts "CHECKSUM:    #{ checksum(input).join('') }"
  end
end

# PART 1 = 11100111011101111
