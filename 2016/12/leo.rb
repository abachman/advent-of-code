...
while pointer > -1 && pointer < INPUT.size
  cycles += 1
  instruction = INPUT[pointer]
  cmd, val, to_reg = instruction.split(' ')
  # puts "INST #{ instruction.inspect }"
  ...
  when 'jnz'
    if regs[val] != 0
      to_reg = to_reg.to_i
      # puts "POINTER JUMP #{ pointer } TO #{ pointer + to_reg }"
      # puts "  #{ regs.inspect }"
      pointer += to_reg
      next
    end
    ...
    raise "UNRECOGNIZED INSTRUCTION #{ instruction.inspect }"
  end

  pointer += 1
end

puts "CYCLES USED: #{ cycles }"
puts "REGISTERS: #{ regs.inspect }"
