...
def triple(str)
  ...
  while i < str.size - 2
    ...
    if STRICT
      return c if (i === 0 || c != str[i - 1]) && # lookbehind 1
                  c === str[i + 1] && # lookahead 1
                  c === str[i + 2] && # lookahead 2
                  c != str[i + 3]     # lookahead 3
    else
      # LOOSE
      return c if c === str[i + 1] && # lookahead 1
                  c === str[i + 2]    # lookahead 2
    end
  ...
  while i < str.size - 4
    ...
    if STRICT
      return c if (i === 0 || c != str[i - 1]) && # lookbehind 1
                  ...
                  c === str[i + 2] && # lookahead 2
                  c === str[i + 3] && # lookahead 3
                  c === str[i + 4] && # lookahead 4
                  c != str[i + 5]     # lookahead 5
    else
      return c if c === str[i + 1] && # lookahead 1
                  c === str[i + 2] && # lookahead 2
                  c === str[i + 3] && # lookahead 3
...
# while the index of the last found key past LIMIT is 1000 less than the current index
while keys.size < LIMIT
  index += 1
  h = hash(salt, index)

  if char = triple(h)
    # puts "TRIPLE #{ char } AT #{ index }"

    # check next 1000
    n = index + 1
    while n < index + 1000
      nh = hash(salt, n)
      if fchar = quintuple(nh)
        if char === fchar
          puts "  > FOUND #{ fchar } FROM #{ index } QUINT AT #{ n }"
          keys << [h, index, nh, n]
          break
        end
      end
...
puts
puts "STOP AT INDEX #{ index }, GOT #{LIMIT}"
puts
puts "KEYS:"
puts
keys.each do |k|
