...
PART_ONE = true
INPUT = open('input.txt').readlines().map(&:strip).join('')
# INPUT = "X(8x2)(3x3)ABCY" # => PART ONE: 18, PART TWO: 20
# INPUT = "(27x12)(20x12)(13x14)(7x10)(1x12)A" # => PART ONE: 324, PART TWO: 241920

def decompress(sequence, level=0)
  puts (' ' * level) + "DECOMPRESS: #{ sequence }"

  index = 0
  output_size = 0

  while index < sequence.size
    if com = /^\((\d+)x(\d+)\)/.match(sequence[index..-1])
      scan_to = com[1].to_i
      repeat  = com[2].to_i
      ...
      output_size += sub_size * repeat

      # skip ahead
      index = index + command_size + scan_to
    else
      output_size += 1
      index += 1
    end
