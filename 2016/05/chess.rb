...
if PART_ONE
  while out.size < 8
    h = md5.hexdigest(door_id + idx.to_s)
    idx += 1
    puts "#{ idx }: #{ out.inspect }" if idx % 10000 == 0
    next unless h.start_with?('00000')
    out << h[5]
  end
else
  found = false
  ...
  while !found
    h = md5.hexdigest(door_id + idx.to_s)
    puts "#{ idx }: #{ out.inspect }" if idx % 100000 == 0
    if h.start_with?('00000')
      pos = h[5]
      if pos >= '0' && pos <= '7' && out[pos.to_i].nil?
        puts "  CHAR AT #{ idx } - #{h} - #{ h[6] }"
        out[pos.to_i] = h[6]
        found = out.none? {|c| c.nil?}
      end
    end
    idx += 1
  end
