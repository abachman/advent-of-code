...
LINE_MATCHER = /([A-Z][a-z]+) would (gain|lose) (\d+) happiness units by sitting next to ([A-Z][a-z]+)/

def parse(line, db)
  if LINE_MATCHER.match(line)
    origin, mode, points, neighbor = [$1, $2, $3, $4]

    db[origin] ||= {}
    db[origin][neighbor] = points.to_i * (mode == 'lose' ? -1 : 1)
  end
end
...
db.keys.permutation do |order|
  ...
  order.each_with_index do |person, idx|
    ...
    layout[person] = db[person][left]
    layout[person] += db[person][right]
  end

  layouts << [layout.values.inject(0, &:+), layout]
end

score, best = layouts.sort_by {|l| l[0]}.last

puts "BEST LAYOUT: #{ best.keys.join(' -> ') }"
puts "SCORE: #{ score }"
