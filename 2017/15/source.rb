...
gena = new_generator_a(a, PART_ONE ? 1 : 4)
genb = new_generator_b(b, PART_ONE ? 1 : 8)

match_count = 0
total_count = 0

puts "%16s  %16s" % ['Gen A', 'Gen B']

(PART_ONE ? 40_000_000 : 5_000_000).times do
  val_a = gena.call & rightmost
  val_b = genb.call & rightmost

  total_count += 1

  if val_a === val_b
    match_count += 1
    puts "-- #{match_count} of #{total_count} --"
    puts "%16s   %16s" % [val_a.to_s(2), val_b.to_s(2)]
  end
end

puts
puts "PART #{PART_ONE ? 'ONE' : 'TWO'}: #{match_count}"
