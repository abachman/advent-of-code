...
input.each do |line|
  cmd, reg, val = line.split(' ')
  instructions << [cmd, reg, val]
end

def runner(instructions, inq, outq, verbose = false)
  sends = 0
  pid = (rand() * 1000).to_i
  puts "#{pid} START" if verbose
  ...
  Enumerator.new do |enum|
    ...
    loop do
      ...
      when 'snd'
        outq << registers[reg]
        sends += 1
        puts "#{pid} SEND #{sends}" if verbose
      when 'set'
        # puts "SET #{reg} #{val}"
        registers[reg] = val
      when 'add'
      ...
      when 'mod'
        ...
        registers[reg] %= val
      when 'rcv'
        # puts "#{pid} RCV #{reg} #{val}" if verbose

        if PART_ONE
          if registers[reg] != 0
            if inq.size > 0
              recv = inq.pop
              puts "  #{pid} RCV #{recv}" if verbose
              enum.yield recv
            else
              inc = 0
              enum.yield nil
          ...
          puts "RECEIVE FROM #{inq.inspect}"
          recv = inq.shift

          if recv.nil?
            puts "  #{pid} WAITING AFTER #{inst}"
            enum.yield inst
            inst = 0
            next
          else
            puts "  #{pid} STORING #{recv} AFTER #{inst}"
            registers[reg] = recv
            inst = 0
          end
        end
...
if PART_ONE
  sounds = []
  program = runner(instructions, sounds, sounds, false)
  sent = program.next
  ...
  puts "QA #{qa.object_id}"
  puts "QB #{qb.object_id}"

  proga = runner(instructions, qa, qb, false)
  progb = runner(instructions, qb, qa, false)
