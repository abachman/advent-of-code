...
# split input into lines of words
...
input.each_with_index do |line, idx|
  ...
  line.each do |word|
    if !PART_ONE
      # in part 2 we're considering anagrams, so just sort every words' letters
      # alphabetically and use that instead
      word = sorted_word(word)
    end

    invalid = !words[word].nil?

    if !words[word].nil?
      puts "  FAIL #{idx+1}: #{word}"
      valid = false
      break
    end
...
puts "#{valid_count} VALID PASSPHRASES"
