...
input_2_groups = "{{<!>},{<!>},{<!>},{<a>}}"
input_score_9 = "{{<ab>},{<ab>},{<ab>},{<ab>}}"
input_score_3 = '{{<a!>},{<a!>},{<a!>},{<ab>}}'
garbage_10 = '<{o"i!a,<{i<a>'

input_actual = open('input.txt').read().strip
# groups are surrounded by {}
# garbage is surrounded by <>, garbage overrides group
# ! is an escape character, anything after is ignored
...
def parse(input)
  ...
  while !c.nil? do
    if cancel
      ...
      case c
      when '{'
        puts (" " * depth) + "GROUP START"
        depth += 1
      when '}'
        group_scores << depth
        puts (" " * depth) + "GROUP END"
        depth -= 1
      when '<'
        garbage = true
      when '!'
  ...
  return [ group_scores, garbage_chars ]
end

# puts parse(input_2_groups).inspect
# puts parse(input_score_9).inspect
# puts parse(input_score_3).inspect
result, garbage_score = parse(input_actual)
score = result.sum

puts result.inspect
puts "----"
puts "SCORE:   #{ score }"
puts "GARBAGE: #{ garbage_score }"
