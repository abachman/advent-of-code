...
# part 1 - Your puzzle answer was 725.
max = 2017
find = 2017
...
while insert <= max
  current = (current + input) % insert

  if PART_ONE
    buffer.insert(current + 1, insert)
  else
    # simulate insertion
    if current === 0
      next_value = insert
    ...
    # if find_index && insert < find_index
    #   find_index += 1
    # elsif current === find_index
    #   next_value = insert
    # end
  end
  ...
  # puts "%i %i %s" % [insert, current, buffer.inspect]
  if insert % 100_000 === 0
    puts "#{ insert } #{ Time.now.to_f - lt }"
    lt = Time.now.to_f
  end
end

if PART_ONE
  idx = buffer.index(find)
  puts buffer[(idx-3)..(idx+3)].inspect
else
  puts "VALUE AT INDEX 1 IS #{next_value}"
end
