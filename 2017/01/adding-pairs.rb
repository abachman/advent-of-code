...
input = open('input.txt').read().strip().chars().map(&:to_i)
PART_ONE = false
...
if PART_ONE
  ...
  input.each_with_index do |i, idx|
    if i === input[(idx + half) % len]
      puts "#{i} matches index #{(idx + half) % len}"
      sum += i
    end
  end
