...
input = open('input.txt').readlines().first.strip.split(' ').map(&:to_i)
# input = %w(0 2 7 0).map(&:to_i)
...
seen = {} # stores each memory configuration
count = 0

puts "INIT   #{input.inspect}"

while seen[input.inspect].nil? do
  seen[input.inspect] = count

  idx = next_bank(input)
  distribute(input, idx)

  count += 1

  puts "NEXT #{ count } #{input.inspect}"
end

steps = count - seen[input.inspect]

puts
puts "  #{count} REPETITIONS BEFORE LOOP"  # part 1
puts "  #{steps} STEPS IN LOOP"            # part 2

# $ ruby source.rb
# INIT   [2, 8, 8, 5, 4, 2, 3, 1, 5, 5, 1, 2, 15, 13, 5, 14]
# NEXT 1 [3, 9, 9, 6, 5, 3, 4, 2, 6, 6, 2, 3, 0, 14, 6, 15]
# NEXT 2 [4, 10, 10, 7, 6, 4, 5, 3, 7, 7, 3, 4, 1, 15, 7, 0]
# ...
# NEXT 3156 [0, 13, 12, 10, 9, 8, 7, 5, 3, 2, 1, 1, 1, 10, 6, 5]
#
#   3156 REPETITIONS BEFORE LOOP
#   1610 STEPS IN LOOP
