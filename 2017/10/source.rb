...
def round(list, input, current, skip)
  ...
  input.each do |length|
    # puts "LENGTH #{ length } CURRENT #{ current }"

    subsection = (0..(length - 1)).inject([]) {|memo, obj|
      memo << list[(current + obj) % list_size]
    }
    # puts "  SUBSECTION #{ subsection.inspect }"
    reversed = subsection.reverse
    # puts "  REVERSED   #{ reversed.inspect }"

    (0..(length - 1)).each do |idx|
      list[(current + idx) % list_size] = reversed[idx]
    end

    # puts "    -> #{ list.inspect }"

    current = (current + length + skip) % list_size
    # puts "  MOVE #{ length } + #{ skip } -> #{ current }"
...
def dense_hash(list)
  ...
  (0..15).each do |block|
    idx = block * 16
    last_idx = ((idx + block_size) - 1)

    # puts "#{ idx } to #{ last_idx }"

    xor = 0
    (idx..last_idx).each do |i|
      # puts "#{ xor } ^ #{ list[i] } = #{ xor ^ list[i] }"
      xor = xor ^ list[i]
    end
...
if PART_ONE
  ...
  64.times do
    # current, skip = round(list, empty_string, current, skip)
    # => a2582a3a0e66e6e86e3812dcb672a272
    # current, skip = round(list, aoc_2017, current, skip)
    # => 33efeb34ea91902bb2f59c9920caa6cd
    current, skip = round(list, actual_input, current, skip)
  end
