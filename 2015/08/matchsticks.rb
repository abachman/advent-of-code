...
def decode(line)
  # drop leading and trailing double quotes
  encoded_line = line.gsub(/(^"|"$)/, '')
  # drop escaped characters
  encoded_line = encoded_line.gsub(/\\"/, '$')
  encoded_line = encoded_line.gsub(/\\\\/, '$')
  encoded_line = encoded_line.gsub(/\\x[a-f0-9]{2}/, '$')
...
in_file.each do |line|
  ...
  # code, string = decode(line)

  #### PART TWO: encode

  code, string = encode(line)

  code_characters += code
  string_characters += string

  puts "  CODE: #{ code }; STRING: #{ string }"
end

puts
puts "FINAL"
puts "STRING: #{ string_characters }; CODE: #{ code_characters }"
puts "ANSWER: #{ code_characters - string_characters }"
