...
def tt(*args)
  args.inject(0) {|sum, val| sum += 2 * val}
end

# in_file = ["2x3x4", "1x1x10"]

in_file.each do |line|
  match, l, w, h = /(\d+)x(\d+)x(\d+)/.match(line).to_a
  l, w, h = [l, w, h].map(&:to_i)

  side_areas = [(2 * l * w), (2 * w * h), (2 * h * l)]
  min_side = side_areas.min / 2
  area = side_areas.inject(0, &:+) + min_side
  # puts "area: lw:%4i wh:%4i hl:%4i min:%6i, area:%6i" % (side_areas + [min_side, area])
  wrapping_paper << area


  side_perimeters = [tt(l, w), tt(w, h), tt(h, l)]
  min_perimeter = side_perimeters.min
  volume = (l * w * h)
  puts "prim: lw:%4i wh:%4i hl:%4i min:%4i, vol: %6i, tot:%6i" % (side_perimeters + [min_perimeter, volume, (min_perimeter + volume)])
  ribbon << min_perimeter + volume
end

puts "#{ wrapping_paper.size } gifts, total required area: #{ wrapping_paper.inject(0, &:+) }"

puts "total required ribbon: #{ ribbon.inject(0, &:+) }"
