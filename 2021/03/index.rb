...
# goal: search list
if PART_TWO
  # return list with only entries having char at idx
  def filter(list, char, idx)
    list.select { |item| item[idx] == char }
  end

  # return most common character in list at idx
  def common(list, idx, most: true)
    zero = 0
    one = 0
    list.each do |item|
      if item[idx] == '0'
  ...
  def update(list, idx, most: true)
    ...
    if list.length > 2
      trace "  #{list.length} items remain"
      ob = list.length
      oc = common(list, idx, most: most)
      trace "  common #{oc}"
      out = filter(list, oc, idx)
      trace "  out len #{out.length} #{out.inspect}"
      if out.length == ob
        raise "filter did not change #{name} length"
      end
    elsif list.length == 2
      trace "  #{list.length} items remain"
      out = [ list[0][idx] == (most ? '1' : '0') ? list[0] : list[1] ]
    else
      trace "#{name}: #{out.join('')}"
    end
  ...
  (cols).times do |idx|
    oxy = update(oxy, idx, most: true)
    co2 = update(co2, idx, most: false)
  end

  puts "OXY: #{oxy[0]}"
  puts "CO2: #{co2[0]}"
  puts "RESULT: #{oxy[0].to_i(2) * co2[0].to_i(2)}"
end
