...
def evolve(s, round=0, quiet=true)
  puts "ROUND #{ round }"
  ...
  char_count = 0
  s.each_char do |cur|
    puts "CHAR #{ char_count } CUR #{ cur } PREV #{ prev }" unless quiet
    char_count += 1
    if prev == nil
      hold << cur
      prev = cur
    ...
    # matches, keep counting
    if cur == prev
      hold << prev
      prev = cur
    else
      # current doesn't match prev, add count & digit to out
      to_add = "#{ hold.size }#{ prev }"
      puts "ADDING #{ to_add }" unless quiet
      out << to_add
  ...
  # didn't hit adder in last round
  if hold.size > 0
    to_add = "#{ hold.size }#{ hold[0] }"
    puts "ADDING #{ to_add }" unless quiet
    out << to_add
  end
